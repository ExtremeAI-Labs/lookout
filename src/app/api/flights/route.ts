...
import { watchlist, type NotableTag } from '@/lib/watchlist';
import { aggregateNavDegradation } from '@/lib/nav-degradation';
import { NavStore, createNavSampler } from '@/lib/nav-sampler';
...
// adsb.fi's published limit is one request per second (github.com/adsbfi/opendata), so
// the regional sweep is paced.
//
// CORRECTION 2026-09-17: this file used to say adsb.fi "soft-throttles by returning 200
// with an empty ac[]". It does not. The geographic endpoint was being called on /v2,
// where — unlike every other v2 endpoint — the list arrives under `aircraft`, not `ac`
// (adsb.fi documents v2/lat/lon/dist as deprecated for exactly that reason). Reading
// `data.ac` therefore always produced [], which looked like throttling and meant the
// "last resort" sweep below could never have returned a single aircraft. The sweep now
// uses the documented /v3 endpoint, and accepts either key.
const ADSBFI_GAP_MS = 1100;
const ADSBFI_GEO_BASE = 'https://opendata.adsb.fi/api/v3';
...
async function fetchAdsbFiRegion(lat: number, lon: number): Promise<any[]> {
  try {
    const res = await stealthFetch(`${ADSBFI_GEO_BASE}/lat/${lat}/lon/${lon}/dist/${ADSB_MAX_DIST}`, {
    ...
    console.warn(`[OSIRIS] adsb.fi region ${lat},${lon} answered HTTP ${res.status}`);
    await res.body?.cancel();
  } catch (e) {
    console.warn(`[OSIRIS] adsb.fi region ${lat},${lon} failed:`, e instanceof Error ? e.message : e);
  }
  return [];
}

// ── Navigation-accuracy sampler (see src/lib/nav-sampler.ts) ─────────────────
// OpenSky stays the worldwide picture; this adds NACp "where it can" without making
// adsb.fi the primary feed. Regions where interference is routinely reported go first,
// so a freshly started console has something meaningful within its first bursts.
const NAV_PRIORITY_REGIONS = [
  { lat: 52.0, lon: 22.0 },  // Eastern Europe / Baltics
  { lat: 44.0, lon: 34.0 },  // Black Sea
  { lat: 33.5, lon: 34.5 },  // Eastern Mediterranean / Levant
  { lat: 39.0, lon: 35.0 },  // Turkey / Aegean
  { lat: 25.0, lon: 45.0 },  // Arabian Gulf
  { lat: 60.0, lon: 15.0 },  // Scandinavia
];
const navStore = new NavStore();
const navSampler = createNavSampler({
  regions: [...NAV_PRIORITY_REGIONS, ...REGIONS.filter(r => !NAV_PRIORITY_REGIONS.some(p => p.lat === r.lat && p.lon === r.lon))],
  fetchRegion: fetchAdsbFiRegion,
  store: navStore,
});
...
function classifyFlight(f: any) {
  ...
  return {
    ...
    nac_p: f.nac_p,
    type: 'flight',
    // filled in by the watchlist when this airframe is on it (see lib/watchlist)
    notable: undefined as NotableTag | undefined,
  };
}
...
export async function GET() {
  ...
  // Fire and forget: advances at most one short burst per five minutes, and only while
  // something is actually asking for flights.
  void navSampler.maybeRun().catch(e => console.warn('[nav-sampler] burst failed:', e instanceof Error ? e.message : e));
  ...
  fetchPromise = (async () => {
    ...
    // state: 30 requests a cycle is more than a polite client should spend on a
    // community feed when it has another source. Paced at ~1 req/s; 30 regions
    // ≈ 33s, inside the 60s maxDuration above. (It returned nothing at all until
    // 2026-09-17 — see the correction at ADSBFI_GAP_MS.)
    if (!openSkyWorked) {
      source = 'regional';
    ...
    const jets: any[] = [];
    const military: any[] = [];
    // Everything adsb.fi-sourced in this cycle (the /mil feed, and the regional sweep when
    // OpenSky is down) joins the rolling accuracy window. OpenSky rows carry no NACp and are skipped.
    const cycleAt = Date.now();
    navStore.ingest(allRaw, cycleAt);

    const flights: NonNullable<ReturnType<typeof classifyFlight>>[] = [];
    for (const raw of allRaw) {
      const flight = classifyFlight(raw);
      if (!flight) continue;
      flights.push(flight);

      // Best of both: an OpenSky aircraft borrows the accuracy adsb.fi recently reported for it.
      if (typeof flight.nac_p !== 'number' && raw.hex) {
        const lent = navStore.nacpFor(String(raw.hex), cycleAt);
        if (lent !== undefined) flight.nac_p = lent;
      }

    }

    // Watchlist: tag the airframes the operator watches (packs + custom) and log sightings.
    // Never blocks the feed: a watchlist failure leaves the poll untagged and a log line.
    let notables: Awaited<ReturnType<ReturnType<typeof watchlist>['observe']>>['notables'] = [];
    try {
      const seen = await watchlist().observe(flights, cycleAt);
      notables = seen.notables;
      if (notables.length) {
        const byHex = new Map(notables.map((n) => [n.hex, n]));
        for (const f of flights) { const n = byHex.get(f.icao24.toLowerCase()); if (n) f.notable = { label: n.label, pack: n.pack, reg: n.reg }; }
      }
    } catch (e) { console.error('[flights] watchlist observe failed', e instanceof Error ? e.message : e); }
    ...
    return {
      ...
      // watched airframes present in this poll (see /api/watchlist)
      notables,
      // Cells where a real share of aircraft report degraded accuracy — see lib/nav-degradation.
      // Computed over the rolling adsb.fi accuracy window (nav-sampler), not just this cycle.
      gps_jamming:        aggregateNavDegradation(navStore.samples(cycleAt)),
      total:              allRaw.length,
      source,
      ...
      providers: {
        ...
        // aircraft currently in the rolling accuracy window, and whether adsb.fi is throttling the sampler
        nav_sampler:     { aircraft: navStore.size, bursts: navSampler.state.bursts, throttled: navSampler.state.throttled, backing_off: Date.now() < navSampler.state.backoffUntil },
      },
