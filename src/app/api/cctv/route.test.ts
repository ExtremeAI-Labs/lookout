...
import { clearSourceCache, seedSource } from '@/lib/sourceCache';
...
vi.mock('./us-traffic', () => ({ fetchCaltransCameras: vi.fn(async () => []), fetchWSDOTCameras: vi.fn(async () => []) }));
vi.mock('@/lib/public-webcams', () => ({ livePublicWebcams: async () => [] }));
vi.mock('@/lib/windy-webcams', () => ({ windyCaliforniaCameras: async () => [], windyUSEastCameras: async () => [], windyUSCentralCameras: async () => [] }));
...
describe('CCTV partial responses', () => {
  it('keeps California coverage when its refresh fails while Washington succeeds', async () => {
    const old = { id: 'cal-1', source: 'Caltrans', feed_url: 'https://cwwp2.dot.ca.gov/a.jpg', lat: 34, lng: -118, name: 'CA', city: 'LA', country: 'US' };
    seedSource('cctv:us-west', [old], 0);
    vi.mocked(fetchCaltransCameras).mockRejectedValue(new Error('Page two failed'));
    vi.mocked(fetchWSDOTCameras).mockResolvedValue([{ ...old, id: 'wsdot-2', source: 'WSDOT' }]);
    await GET(new Request('http://localhost/api/cctv?region=us-west'));
    await flushIO();
    const body = await (await GET(new Request('http://localhost/api/cctv?region=us-west'))).json();
    expect(body.cameras.map((c: { id: string }) => c.id)).toEqual(expect.arrayContaining(['cal-1', 'wsdot-2', 'site-port-la-waterfront']));
    const live = await (await GET(new Request('http://localhost/api/cctv?region=us-west&live=1'))).json();
    expect(live.cameras).toEqual([]); // snapshots and operator pages never appear as live feeds
  });
