...
import { cachedSource, isStale, peekSource, seedSource, DEFAULT_TTL_MS } from '@/lib/sourceCache';
...
import { usableCameras, isLiveCamera } from '@/lib/cctv-usable';
import { livePublicWebcams } from '@/lib/public-webcams';
import { windyCaliforniaCameras, windyUSEastCameras, windyUSCentralCameras } from '@/lib/windy-webcams';
import { createPool } from '@/lib/fetch-pool';
import { fetchCaltransCameras, fetchWSDOTCameras } from './us-traffic';
import { PUBLIC_CAMERA_SITES } from '@/lib/public-camera-sites';
...
/** A provider outage must not remove its pins while the other western sources refresh. */
async function retainWesternTraffic(source: string, fetcher: RegionFetcher): Promise<any[]> {
  try {
    const cameras = await fetcher();
    if (!cameras.length) throw new Error('Empty traffic source');
    return cameras;
  } catch {
    return (peekSource<any>('cctv:us-west', true) ?? []).filter(camera => camera.source === source);
  }
}
...
const RAW_REGION_FETCHERS: Record<string, RegionFetcher> = {
  ...
  'uk': fetchTfLCameras,
  'us-west': async () => { const [w, c, pub, windy] = await Promise.all([retainWesternTraffic('WSDOT', () => fetchWSDOTCameras()), retainWesternTraffic('Caltrans', () => fetchCaltransCameras()), livePublicWebcams(), windyCaliforniaCameras()]); return [...w, ...c, ...pub, ...windy, ...PUBLIC_CAMERA_SITES]; },
  'us-east': async () => { const [base, windy] = await Promise.all([fetchUSEastCameras(), windyUSEastCameras()]); return [...base, ...windy]; },
  'us-central': async () => { const [base, windy] = await Promise.all([fetchUSCentralCameras(), windyUSCentralCameras()]); return [...base, ...windy]; },
...
function rebuildPayload(): Payload | undefined {
  ...
  for (const region of ALL_REGIONS()) {
    /* Entries with a non-URL feed value, and sources measured as hard-blocked, are dropped here so
       they never reach the map as permanently-broken pins. See lib/cctv-usable.ts. */
    for (const camera of usableCameras(regions[region] ?? [])) {
      cameras.push(camera);
...
function ensureRestored(): Promise<void> {
  restoring ??= (async () => {
    ...
    for (const [region, list] of Object.entries(saved.regions)) {
      if (region in REGION_FETCHERS) {
        // A restart does not make an old catalog fresh again.
        const age = Math.max(0, Date.now() - (Number(saved.builtAt) || 0));
        seedSource(`cctv:${region}`, list, Math.max(0, DEFAULT_TTL_MS - age));
        cameras += list.length;
      }
    }
    rebuildPayload();
...
export async function GET(request: Request) {
  try {
    ...
    /* ?live=1 narrows to real video feeds. 88% of the catalogue is snapshot stills, so this is how
       he finds the ~1,770 cameras that actually stream. */
    const liveOnly = searchParams.get('live') === '1';

    if (regionsToFetch.length === ALL_REGIONS().length && !liveOnly) {
    ...
    for (const region of regionsToFetch) {
      for (const cam of usableCameras(collected.cameras[region] ?? [])) {
        allCameras.push(cam);
