...
import { isWindyStreamUrl } from '@/lib/camera-feed';
import { extractWindyInnerPlayer, isBlockedStreamHost } from '@/lib/windy-webcams';
...
type Resolution =
  ...
  // Windy stream pages wrap the operator's own player (ipcamlive, brownrice, a resort's page…);
  // the page is the allowlisted host, the inner URL is whatever the operator published.
  | { embeddable: true; kind: 'windy'; embedUrl: string }
...
function isResolvable(url: string): boolean {
  ...
  if (isWindyStreamUrl(url)) return true;
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (res.status === 404 || res.status === 410) {
      ...
      if (isWindyStreamUrl(url)) {
        const inner = extractWindyInnerPlayer(html);
        // A Caltrans inner player is real video we cannot reach from this network: say so
        // ('hls' → the viewer offers the source link) instead of an iframe that never starts.
        value = inner && !isBlockedStreamHost(inner)
          ? { embeddable: true, kind: 'windy', embedUrl: inner }
          : { embeddable: false, kind: inner ? 'hls' : 'unknown' };
      } else if (isSkylineUrl(url)) {
