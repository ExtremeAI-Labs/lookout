...
export interface AircraftDetail {
  ...
  /** Unix seconds at each track point, aligned by index (null when the trace carries no clock). */
  times?: (number | null)[];
  /** Ground speed in knots at each track point, aligned by index. */
  speeds?: (number | null)[];
  points: number;
...
export function parseTrace(
  ...
  const t0 = typeof json?.timestamp === 'number' ? json.timestamp : null;
  ...
  for (const row of rows) {
    ...
    // readsb rows start with seconds since the file's timestamp; ground speed is column 4.
    const dt = row?.[0];
    times.push(t0 !== null && typeof dt === 'number' ? Math.round(t0 + dt) : null);
    const gs = row?.[4];
    speeds.push(typeof gs === 'number' ? gs : null);
  }
...
export async function GET(request: Request) {
  try {
    ...
    // ?max= raises the point cap for the 3D theater's stacked-loop drawing (default suits the flat map).
    const maxPoints = Math.min(3000, Math.max(50, Number(searchParams.get('max')) || 700));
    ...
    const detail = await cachedSource<AircraftDetail>(
      `aircraft:${icao24}:${full ? 'full' : 'recent'}:${legOnly ? 'leg' : 'all'}:${maxPoints}`,
      async () => {
        ...
        const parsed = trace ? parseTrace(trace, maxPoints, legOnly) : null;
