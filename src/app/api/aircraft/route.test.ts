...
describe('parseTrace clock and speed columns (theater replay)', () => {
  const file = { icao: 'ABC123', r: 'N1', t: 'B738', timestamp: 1_700_000_000, trace: [
    [0, 34.0, -118.0, 1000, 250, 90, 0], [10, 34.1, -118.1, 2000, 260, 90, 0], [20, 34.2, -118.2, 'ground', 0, 90, 0],
  ] as Array<Array<number | string | null>> };
  it('carries unix seconds and knots aligned with the track', () => {
    const p = parseTrace(file, 700, false);
    expect(p.times).toEqual([1_700_000_000, 1_700_000_010, 1_700_000_020]);
    expect(p.speeds).toEqual([250, 260, 0]);
    expect(p.altitudes).toEqual([1000, 2000, null]);
  });
  it('keeps times aligned through the downsample and honours the cap', () => {
    const rows = Array.from({ length: 3000 }, (_, i) => [i * 5, 34 + i / 1e4, -118 - i / 1e4, 5000, 200, 90, 0]) as Array<Array<number | string | null>>;
    const p = parseTrace({ icao: 'abc123', timestamp: 1_700_000_000, trace: rows }, 100, false);
    expect(p.points).toBe(100);
    expect(p.times).toHaveLength(100);
    expect(p.times?.[99]).toBe(1_700_000_000 + 2999 * 5);
    expect(p.track[99][1]).toBeCloseTo(34 + 2999 / 1e4, 6);
  });
  it('reports null times when the file has no timestamp', () => {
    const p = parseTrace({ icao: 'abc123', trace: file.trace }, 700, false);
    expect(p.times).toEqual([null, null, null]);
  });
});
