 ...
 */
/** A Windy stream page: it wraps the operator's real player, which the resolver extracts. */
export function isWindyStreamUrl(url: string | undefined | null): boolean {
  if (!url) return false;
  try { const u = new URL(url); return u.hostname === 'webcams.windy.com' && /^\/webcams\/stream\/\d+\/?$/.test(u.pathname); } catch { return false; }
}

/**
 * Windy live cameras carry the stream PAGE as their stream_url. It plays as-is (the operator's
 * player sits inside it), but resolving to that inner player lets our own iframe grant autoplay,
 * so the stream starts without a click and never leaves the viewer.
 */
export function windyStreamKey(camera: ResolvableCamera | null | undefined): string | null {
  return camera?.stream_type === 'iframe' && isWindyStreamUrl(camera.stream_url) ? camera.stream_url! : null;
}
